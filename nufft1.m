function f = nufft1(c, x, y, w, n)
% 二维 nufft 求解器, 采用 Guassian 作为 spreading function
...
for l_1 = 0:1:n-1
    for l_2 = 0:1:n-1
        for j = 1:1:M 
            b(l_1+1, l_2+1) = b(l_1+1, l_2+1) + c(j) * phi1(alpha, beta, l_1*h - x(j), l_2*h - y(j));
        end
    end
end
...
for k_1 = 0:1:n-1
    for k_2 = 0:1:n-1
        p = h^2 / (pi * alpha^2 / beta * exp(-alpha^4 * ((-n/2 + k_1)^2 + (-n/2 + k_2)^2) / (4 * beta)));
